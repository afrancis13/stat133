...
source("~/Downloads/computeSJDistance.R")
...
speeches <- readLines(con=file("~/Downloads/stateoftheunion1790-2012.txt"))
...
# Answer is no, final line *** END OF COMPLETE ADDRESSES ***
# We need to account for this when we compute n.speeches, so subtract 1.

breaks <- grep(pattern="\\*\\*\\*", x=speeches)
...
for (i in 1:n.speeches) {
    presidents <- c(presidents, speeches[breaks[i]+3])
}
...
tempDates <- c()
for (i in 1:n.speeches) {
    tempDates <- c(tempDates, speeches[breaks[i]+4])
}

speechYr <- c()
speechMo <- c()
for (i in 1:length(tempDates)) {
    splitted <- strsplit(x=tempDates[i], split=" ")
    speechMo <- c(speechMo, splitted[[1]][1])
    speechYr <- c(speechYr, splitted[[1]][3])
}
...
# THIS MIGHT BE A LITTLE OFF - [[222]][[222]] (why repeat??)
speechesL <- list()
for(i in 1:n.speeches){
    thisSpeech <- c()
    for(j in (breaks[i]:breaks[i+1]-1)) {
        thisSpeech <- c(thisSpeech, speeches[j])
    }
    thisSpeech <- paste(x=thisSpeech, collapse=" ")
    splittedSpeech <- strsplit(x=thisSpeech, split="[.?!]")
    speechesL[[i]] <- unlist(splittedSpeech)
}
...
speechToWords = function(sentences) {
    ...
    newSentences <- gsub("'", "", sentences)
    newSentences <- gsub("[0-9]+s*", replacement="", x=sentences)
    newSentences <- tolower(newSentences)
    
    # Drop the words (Applause. and Laughter.)
    newSentences <- gsub("\\(applause.\\)", replacement="", x=newSentences)
    newSentences <- gsub("\\(laughter.\\)", replacement="", x=newSentences)
    
    # Split the text up by blanks and punctuation  (hint: strsplit, unlist)
    newSentences <- tolower(newSentences)
    
    wordsList <- strsplit(x=newSentences, split="[[:punct:][:blank:]]+")
    
    wordsUnlist <- unlist(wordsList)
    
    # Drop any empty words
    wordsUnlist <- wordsUnlist[wordsUnlist != ""]
    #wordsVec <- c()
    #for (i in 1:length(wordsList[[1]])) {
    #  if (wordsList[[1]][i] != "") {
    #    wordsVec <- c(wordsVec, wordsList[[1]][i])
    #  }
    #}
...
#    thisSpeech <- c(thisSpeech, speeches[j])
#  }
#  speechWords[[i]] <- paste(thisSpeech, collapse = " ")
#}

parsedSpeeches <- sapply(X=speechesL, FUN=speechToWords)
...
unlisted <- unlist(parsedSpeeches)
uniqueWords <- sort(unique(unlisted))
...
wordVecs <- lapply(parsedSpeeches, function(x){
    tablex = table(x)
    vec = emptyVec
    vec[names(tablex)] = tablex
    return(vec)
})
...
wordMat <- matrix(data=unlist(wordVecs), nrow=no.uniqueWords, byrow=FALSE)
...
load("~/Downloads/speeches_dataframe.Rda")
...
sentences <- c()
for (i in 1:n.speeches) {
    words <- c(words, length(parsedSpeeches[[i]]))
    chars <- c(chars, sum(nchar(parsedSpeeches[[i]])))
    sentences <- c(sentences, length(unlist(speechesL[[i]])))
}
...
transpose <- t(wordMat)
split_transpose <- split(data.frame(transpose), speechesDF$Pres)
sum_split_transpose <- sapply(split_transpose, colSums)
presidentWordMat <- matrix(sum_split_transpose, ncol=length(unique(presidents)))
...
docFreq <- rowSums(presidentWordMat != 0)
...
sortedPres <- sort(unique(presidents))
presParty <- sapply(sortedPres, function(x) unique(speechesDF$party[speechesDF$Pres == x]))
names(presParty) <- sortedPres
...
text(mds, col=cols, labels=names(presParty))
...
plot(x = speechesDF$yr, y = speechesDF$sent, xlab = "Year", ylab = "Number of Sentences", main = "State of The Union Sentence Count, 1790 - 2012")
plot(x = speechesDF$yr, y = speechesDF$words, xlab = "Year", ylab = "Number of Words", main  = "State of The Union Word Count, 1790 - 2012")
plot(x = speechesDF$yr, y = speechesDF$chars, xlab = "Year", ylab = "Number of Characters", main = "State of The Union Character Count, 1790 - 2012")
plot(x = speechesDF$yr, y = speechesDF$chars/speechesDF$words, xlab = "Year", ylab = "Number of Characters/Word ", main = "State of the Union Number of Characters/Word, 1790 - 2012")
plot(x = speechesDF$yr, y = speechesDF$words/speechesDF$sent, xlab = "Year", ylab = "Number of Words/Sentence", main = "State of the Union Number of Words/Sentences, 1790 - 2012")
